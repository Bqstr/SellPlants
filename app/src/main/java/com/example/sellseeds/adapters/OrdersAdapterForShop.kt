...
class OrdersAdapterForShop (val navConteoller: NavController, val context: Context, var isBuyer:Boolean,  ): RecyclerView.Adapter<OrdersAdapterForShop.OrderViewHolder>(){
    ...
    override fun onBindViewHolder(holder: OrderViewHolder, position: Int) {
        ...
        with(holder.binding){
            ...
            when(order.status){
                OrderStatus.Completed ->{txtCompleted.setTextColor(
                    ContextCompat.getColor(context!!,
                    R.color.greeeen
                ))}
                OrderStatus.Canceled ->{txtCompleted.setTextColor(
                    ContextCompat.getColor(context!!,
                    R.color.red
                ))}
                OrderStatus.InProgress->{txtCompleted.setTextColor(
                    ContextCompat.getColor(context!!,
                    R.color.grey
                ))}
